...
name: computer-use
description: >
  主Agent专用桌面操控，优先通过无障碍接口操作，仅在无法触及时使用视觉回退。。Main-agent-only desktop control through accessibility-first semantic actions with visual pixel fallback only when accessibility cannot reach the target
---
